...
<template>
  ...
  <!-- <el-header class="header aagames-nav-bar"> -->
    <div class="aagames-login-header">
      ...
      <div class="aagames-nav-bar lheader" :style="{position:isPopup?'absolute':''}">
        <div class="nav-left">
          <template v-if="goback">
            <template v-if="(step >2 || steps > 1) && !isloginReg && step != 10">
              <van-icon name="arrow-left" @click="go" />
            </template>
            <template v-else-if="$projectname !== '10050' && (step == 2 || steps == 1 || isloginReg || step == 10)">
      ...
      <div class="bg-login" v-if="isBanner">
        ...
        <img class="cooperation" v-if="$projectname=='wanqu'||$projectname=='chashao'" :src="require('@assets/img3_0/login/cooperation@2x.png')" alt="">
