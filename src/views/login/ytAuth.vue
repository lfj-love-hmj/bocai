    ...
    <div class="container">
      <div class="main">
        <div class="auth-container">
          <div>
            <img class="wanqu" src="@assets/img3_0/auth/wanqu_logo.png" alt />
            <p>{{$config[$projectname].secName}}</p>
          </div>
          <div>
            <img class="arrow" src="@assets/img3_0/auth/right_arrow.png" alt />
          </div>
          <div>
            <img class="youtiao" src="@assets/img3_0/auth/youtiao_logo.png" alt />
            <p>油条平台</p>
          </div>
        </div>
        <div class="auth-bottom">
          <h4>同意将{{$config[$projectname].project}}账户的以下信息授权给油条</h4>
          <ul>
            <li>
              获取你的登录状态
              <img src="@assets/img3_0/auth/checked.png" alt />
            </li>
            <li>
              获取你的公开信息（昵称、头像）
              <img src="@assets/img3_0/auth/checked.png" alt />
...
export default {
  ...
  methods: {
    submit() {
      ...
      const ytDomain = sessionStorage.getItem('ytDomain').split('//')[1]
      const params = {
      ...
      authlogin(params).then(res => {
        ...
        setTimeout(()=> {
          window.location.href= `m.${ytDomain}/auth?at=${res.data.data.access_token}`;
        },300)
      })
      // axios({
      //   method: "post",
    ...
    handlerAuth() {
      ...
      if (this.$route.query.yt_domain) {
        sessionStorage.setItem("ytDomain", this.$route.query.yt_domain);
      }
      if (this.token) {
        this.isAuth = true;
      } else if (!this.token) {
